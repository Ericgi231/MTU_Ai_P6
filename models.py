...
class RegressionModel(object):
    ...
    def __init__(self):
        ...
        self.weight0 = nn.Parameter(1, 50)
        self.weight1 = nn.Parameter(50, 1)
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        xweight1 = nn.Linear(x, self.weight0)
        r1 = nn.ReLU(nn.AddBias(xweight1, self.bias0))
        xweight2 = nn.Linear(r1, self.weight1)
        return nn.AddBias(xweight2, self.bias1)
    ...
    def train(self, dataset):
        ...
        while change:
            ...
            for x, y in dataset.iterate_once(self.batchSize):
                ...
                grad = nn.gradients(loss, [self.weight0, self.weight1, self.bias0, self.bias1])
                self.weight0.update(grad[0], self.learnRate)
                self.weight1.update(grad[1], self.learnRate)
                self.bias0.update(grad[2], self.learnRate)
                self.bias1.update(grad[3], self.learnRate)
